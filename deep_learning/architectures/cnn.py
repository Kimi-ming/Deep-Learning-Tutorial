...
注意: 此文件已作为兼容入口，推荐使用 `from deep_learning.architectures import SimpleCNN`
"""
...
warnings.warn(
    ...
    "请使用 from deep_learning.architectures import SimpleCNN",
    DeprecationWarning,
    stacklevel=2,
)
...
def conv_operation_demo():
    ...
    # 边缘/锐化卷积核示例
    kernels = {
        "laplacian": PREDEFINED_KERNELS["laplacian"],
        "sobel_x": PREDEFINED_KERNELS["sobel_x"],
        "sharpen": PREDEFINED_KERNELS["sharpen"],
    }
    ...
    for name, edge_kernel in kernels.items():
        print(f"\n{name} 卷积核 (3x3):")
        ...
        print(f"\n{name} 卷积输出 ({len(output)}x{len(output[0])}):")
        for row in output:
...
def forward_batch(model, batch_inputs):
    ...
    批量前向辅助函数（逐样本执行，教学用途）
    """
    return [model.forward(sample) for sample in batch_inputs]
