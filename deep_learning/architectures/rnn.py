...
warnings.warn(
    "deep_learning_rnn.py 将迁移到 deep_learning/architectures/ 包，"
...
class SimpleRNN:
    ...
    def forward(self, inputs, initial_hidden=None):
        ...
        for input_vec in inputs:
            ...
            h_input = [
                sum(self.Wxh[k][j] * input_vec[k] for k in range(self.input_size))
                for j in range(self.hidden_size)
            ]
            h_hidden = [
                sum(self.Whh[k][j] * hidden[k] for k in range(self.hidden_size))
                for j in range(self.hidden_size)
            ]
            ...
            for j in range(self.output_size):
                s = sum(self.Why[k][j] * hidden[k] for k in range(self.hidden_size))
                s += self.by[j]
                y_input.append(s)

            output = softmax(y_input)
    ...
    def forward_batch(self, batch_inputs):
        """批量前向（逐样本计算）"""
        return [self.forward(seq)[0] for seq in batch_inputs]

    def apply_gradients(self, dWxh, dWhh, dWhy, dbh, dby, max_norm=5.0):
        """
        应用梯度更新（示例），会对梯度做裁剪防止爆炸。

        Args:
            dWxh, dWhh, dWhy: 与权重形状一致的梯度矩阵
            dbh, dby: 与偏置形状一致的梯度向量
            max_norm: 梯度裁剪阈值
        """
        # 展平梯度后裁剪，再按原形状还原
        flat_grads = (
            [g for row in dWxh for g in row] +
            [g for row in dWhh for g in row] +
            [g for row in dWhy for g in row] +
            list(dbh) + list(dby)
        )
        clipped = clip_gradients(flat_grads, max_norm=max_norm)
        idx = 0
        ...
        def pop_mat(mat):
            ...
            new = []
            for row in mat:
                new_row = clipped[idx: idx + len(row)]
                idx += len(row)
                new.append(new_row)
            return new
        ...
        dWhh_c = pop_mat(dWhh)
        dWhy_c = pop_mat(dWhy)
        dbh_c = clipped[idx: idx + len(dbh)]; idx += len(dbh)
        dby_c = clipped[idx: idx + len(dby)]

        # 梯度下降更新
        for i in range(len(self.Wxh)):
            for j in range(len(self.Wxh[0])):
                self.Wxh[i][j] -= self.learning_rate * dWxh_c[i][j]
        for i in range(len(self.Whh)):
            for j in range(len(self.Whh[0])):
                self.Whh[i][j] -= self.learning_rate * dWhh_c[i][j]
        for i in range(len(self.Why)):
            for j in range(len(self.Why[0])):
                self.Why[i][j] -= self.learning_rate * dWhy_c[i][j]
        for i in range(len(self.bh)):
            self.bh[i] -= self.learning_rate * dbh_c[i]
        for i in range(len(self.by)):
            self.by[i] -= self.learning_rate * dby_c[i]
