...
## 🟡 Phase 3: 测试骨架 (依赖 Phase 2，确保包结构已定)

### 测试框架搭建
- [ ] 创建 tests/ 目录和 tests/__init__.py
- [ ] 配置 pytest.ini (testpaths=tests, python_files=test_*.py)
- [ ] 创建 tests/test_smoke.py (最小 smoke tests)
...
### 最小单元测试 (针对当前结构)
- [ ] tests/test_fundamentals.py (感知机AND门收敛、XOR不收敛)
- [ ] tests/test_cnn.py (卷积输出形状、池化值验证)
- [ ] tests/test_rnn.py (隐藏状态形状、序列处理)
...
## 🟢 Phase 4: 代码重构 (依赖 Phase 3，测试保护下重构)

### 公共工具提取
- [ ] 创建 deep_learning/utils/__init__.py
- [ ] deep_learning/utils/activations.py
  - sigmoid (数值稳定, 范围裁剪 [-500, 500])
  - relu, tanh, softmax (数值稳定处理)
  - 每个函数包含 docstring + 数学公式
- [ ] deep_learning/utils/losses.py
  - mse, cross_entropy, binary_cross_entropy
  - epsilon 参数防止 log(0)
- [ ] deep_learning/utils/initializers.py (xavier, he, uniform)
- [ ] deep_learning/utils/math_ops.py (矩阵乘法、梯度裁剪、归一化)
...
- [ ] 实现 tests/utils/test_activations.py (边界测试、数值稳定性)
- [ ] 实现 tests/utils/test_losses.py (epsilon处理、梯度验证)
- [ ] 更新已有测试使用新 utils
- [ ] 验证所有测试通过
...
|-------|---------------------|---------------------|---------|------|
| Phase 1: 基础与规范 | 10项 | 6项 | 4-6h | 待开始 |
| Phase 2: 包结构与入口 | 5项 | 9项 | 4-6h | 待开始 |
| Phase 3: 测试骨架 | 11项 | 12项 | 6-8h | 待开始 |
| Phase 4: 代码重构 | 15项 | 11项 | 1-2天 | 待开始 |
| Phase 5: 模块迁移 | 15项 | 11项 | 2-3天 | 待开始 |
| Phase 6: 示例与可视化 | 18项 | 10项 | 1-2天 | 待开始 |
| Phase 7: 增强功能 | 9项 | 2项 | 1天 | 待开始 |
| **核心任务总计** | **83项** | **61项细项** | **6-8天** | **0%** |
